...
def main(input_path, input_number, input_bpart, input_cpart,
    ...
    sm_avq_sq = np.mean(transforms.smooth_function(block_sq, 3), axis=0)
    err_sq = np.sqrt(
    ...
    cr_swtch, r_swtch = transforms.sq_to_cr(
        r_bins, input_density, block_sq, q_fft)
    ...
    sqt = np.mean(block_sq, axis=0)
    plt.plot(q_sam, np.square(sqt-1)/sqt)

    # c(r) by fourier inversion for comparision
    cr_fft_m = transforms.sq_and_hr_to_cr(r_bins, input_density, block_rdf-1, r, block_sq, q_sam)
    cr_fft = transforms.hr_to_cr(r_bins, input_density, block_rdf - 1, r)
    avg_cr_fft = np.mean(cr_fft, axis=0)
    avg_cr_fft_m = np.mean(cr_fft_m, axis=0)
    ...
    axes[0, 2].fill_between(r, avg_cr_swtch + err_cr_swtch,
                            avg_cr_swtch - err_cr_swtch, alpha=0.2)
    axes[0, 2].plot(r, avg_cr_fft, label='$c_{fft}(r)$')
    axes[0, 2].plot(r, avg_cr_fft_m, label='$c_{fft_m}(r)$')
    ...
    #                 label='$S_{avg}(q)$')
    axes[1, 0].fill_between(q_sam, avg_sq + err_sq,
                            avg_sq - err_sq, alpha=0.2)
    axes[1, 0].fill_between(q_fft, avg_sq_fft + err_sq_fft,
                            avg_sq_fft - err_sq_fft, alpha=0.2)
    ...
    axarr[0].plot(q_sam, avg_sq, linewidth=1,
                  marker='x', label='$S_{dir}(q)$')
    axarr[0].plot(q_sam, sm_avq_sq, linewidth=1,
                  marker='x', label='$S_{dir}(q)$')
    axarr[0].fill_between(q_sam, avg_sq + err_sq,
                            avg_sq - err_sq, alpha=0.2, color='tab:orange')
    ...
    axarr[0].legend()
    axarr[1].plot(q_sam, (- avg_sq + avg_sq_fft) , 
