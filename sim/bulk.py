...
def main():
    ...
    if not os.path.exists(path + '/temperature'):
        os.mkdir(path + '/temperature')

    # Save output
    output_path = path + '/output/'
    output = np.column_stack((r, phi, avg_rdf, err_rdf, avg_cr_swtch, err_cr_swtch,
                            # 0   1      2        3          4             5
                              q_fft, avg_cr_fft, err_cr_fft, avg_sq_switch, err_sq_switch, 
                            #   6         7           8           9               10     
                              sq_pad, avg_sq_fft, switch_extend))
                            #   11        12            13
    np.savetxt(output_path + '/output_' + test_number + '.dat', output)

    # Save temperature
    temp_path = path + '/temperature/'
    temp_series = np.column_stack((t, kinetic_temp))
    np.savetxt(temp_path + '/temp_' + test_number + '.dat', temp_series)
